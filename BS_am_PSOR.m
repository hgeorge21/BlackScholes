function values = BS_am_PSOR(Sa, Sb, E, r, sigma, T, type, ns, nt, d)
...
for i = nt-1:-1:1
    ...
    for j = 1:ns
        ...
        if type == "put",  payoffs(j, 1) = max(0, E-j*ds); end
        if type == "call", payoffs(j, 1) = max(0, j*ds-E); end
    end
    
    uj = PSOR(Aj, bj, values(i*ns+1:(i+1)*ns,1), 1, payoffs);
